...
export default function QuestionEditor({ open, onClose, onSave, editQuestion }: Props) {
  ...
  const handleSave = () => {
    ...
    if (!isValid) {
      ...
      if (currentErrors.some(k => k === 'question_text' || k === 'module' || k === 'difficulty' || k === 'skill')) {
        setStep(0);
      } else if (currentErrors.some(k => k.startsWith('option') || k === 'options' || k === 'correct_answer')) {
        setStep(1);
      } else {
        setStep(2);
      }
            ...
            <div className="px-6 pb-4 shrink-0">
              <div className="flex items-center gap-2">
                {stepLabels.map((label, i) => (
                  ...
                  >
                    <div
                      className={`flex items-center gap-2 flex-1 py-2 px-3 rounded-xl transition-all text-xs font-bold ${
                        i === step
                      ...
                      >
                        ...
                        >
                          <span
                            ...
                            style={{ left: form.is_free_response ? '18px' : '2px' }}
                          />
                        </button>
                        <div>
                  ...
                  >
                    {form.is_free_response ? (
                      <div>
                        ...
                        {errors.correct_value && (
                          <p className="text-[11px] text-red-400 mt-1 flex items-center gap-1">
                            <span>⚠</span> {errors.correct_value}
                          </p>
                        )}
                      ...
                      <div>
                        ...
                        {errors.correct_answer && (
                          <p className="text-[11px] text-red-400 mt-2 flex items-center gap-1">
                            <span>⚠</span> {errors.correct_answer}
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2 flex-wrap mb-3">
                        ...
                        <span
                          className={`text-[10px] font-bold px-3 py-1 rounded-full border capitalize ${
                      ...
                      {(form.passage_text.trim() || form.passage_image_url.trim()) && (
                        <div className="bg-white/[0.03] rounded-xl p-4 mb-3 border border-white/5">
                          {form.passage_image_url.trim() && (
                            <figure className="mb-4 overflow-hidden rounded-xl border border-white/8 bg-white/5">
                              <img
                                src={form.passage_image_url}
                                alt={form.passage_image_alt || 'Passage image'}
